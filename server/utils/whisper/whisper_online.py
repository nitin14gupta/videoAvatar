...
#        logging.getLogger("faster_whisper").setLevel(logger.level)
        ...
        if use_cuda:
            logger.info(f"CUDA available, using GPU for Whisper model: {model_size_or_path}")
            try:
                # Try GPU with float16 first (fastest)
                model = WhisperModel(model_size_or_path, device="cuda", compute_type="float16", download_root=cache_dir)
                logger.info("Whisper model loaded on GPU with float16")
            except Exception as e:
                logger.warning(f"Failed to load on GPU with float16: {e}. Trying int8_float16...")
                try:
                    ...
                    logger.info("Whisper model loaded on GPU with int8_float16")
                    ...
                    logger.info("Whisper model loaded on CPU with int8")
        else:
            logger.info(f"CUDA not available, using CPU for Whisper model: {model_size_or_path}")
